export default function filtreProgramme() {
  ...
  this.filtreAll = function (data, filtre, tab) {
    data.forEach(itemData => {
      ...
      if ((jourSelected[0] == filtre.jour || filtre.jour == "Tous") && itemData.heureF >= filtre.heure) {
        //FILTRAGE PAR DONNEES TEMPORELLES

        if ((itemData.scene == filtre.lieux || filtre.lieux == "Tous") && (itemData.type == filtre.type || filtre.type == "Tous")) {
          //FILTRAGE PAR LIEUX ET TYPE
          tab.push(itemData);
        }
      }
